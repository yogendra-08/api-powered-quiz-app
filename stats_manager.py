"""
stats_manager.py - Manage quiz statistics persisted to CSV

Provides a simple StatsManager that stores quiz summaries to
`data/quiz_history.csv` and computes basic statistics using
pandas/numpy. This file replaces an incorrect duplicate of the GUI.
"""
import os
import json
...
DEFAULT_CSV = os.path.join("data", "quiz_history.csv")


class StatsManager:
    """Manage quiz history and compute summary statistics.

    Public methods used by the GUI:
    - save_quiz_result(summary: dict)
    - get_total_quizzes() -> int
    - get_statistics_summary() -> dict
    - get_recent_quizzes(n: int) -> pandas.DataFrame
    """

    def __init__(self, csv_path: str = DEFAULT_CSV):
        self.csv_path = csv_path
        os.makedirs(os.path.dirname(self.csv_path), exist_ok=True)
        # Ensure file exists with expected columns
    ...
    def _read_df(self) -> pd.DataFrame:
        try:
            ...
            # If anything goes wrong reading, return empty DataFrame
            df = pd.DataFrame(columns=[
                "date",
                "category",
                "difficulty",
    ...
    def save_quiz_result(self, summary: Dict) -> None:
        """Append a quiz summary to the CSV.

        The `summary` dict is expected to use the shape returned by
        `QuizApp.get_quiz_summary()` from `quiz_app.py`.
        """
        # Normalize row
        row = {
            ...
            "incorrect_answers": int(summary.get("incorrect_answers", 0)),
            "score_percentage": float(summary.get("score_percentage", 0.0)),
            "time_taken_seconds": int(summary.get("time_taken_seconds", 0)),
            "user_answers": json.dumps(summary.get("user_answers", [])),
        }

        df_row = pd.DataFrame([row])

        # Append to CSV; write header only if file is empty
        write_header = not os.path.exists(self.csv_path) or os.path.getsize(self.csv_path) == 0
        df_row.to_csv(self.csv_path, mode="a", index=False, header=write_header)

    def get_total_quizzes(self) -> int:
        df = self._read_df()
        return len(df)

    def get_recent_quizzes(self, n: int = 5) -> pd.DataFrame:
        df = self._read_df()
        if df.empty:
            return df
        ...
        if "date" in df.columns:
            ...
            df_sorted = df
        # Try to convert numerical columns to correct dtypes for display
        for col in ["score_percentage"]:
            if col in df_sorted.columns:
                df_sorted[col] = pd.to_numeric(df_sorted[col], errors="coerce")
        return df_sorted.head(n)

    def get_statistics_summary(self) -> Dict:
        """Return a dictionary with overall statistics used by the GUI.

        Keys returned (matching what `main.py` expects):
        - total_quizzes
        ...
        df["score_percentage"] = pd.to_numeric(df.get("score_percentage", pd.Series()), errors="coerce").fillna(0)
        df["total_questions"] = pd.to_numeric(df.get("total_questions", pd.Series()), errors="coerce").fillna(0).astype(int)
        df["correct_answers"] = pd.to_numeric(df.get("correct_answers", pd.Series()), errors="coerce").fillna(0).astype(int)

        total_quizzes = len(df)
        total_questions_answered = int(df["total_questions"].sum()) if "total_questions" in df.columns else 0

        scores = df["score_percentage"]
        mean_score = float(np.round(scores.mean(), 2)) if not scores.empty else 0.0
        median_score = float(np.round(scores.median(), 2)) if not scores.empty else 0.0

        # Mode: use pandas.Series.mode (may return multiple values); pick first
        try:
            mode_vals = scores.mode()
            mode_score = float(mode_vals.iloc[0]) if not mode_vals.empty else float(scores.iloc[0])
        except Exception:
            mode_score = float(scores.iloc[0]) if not scores.empty else 0.0

        std_deviation = float(np.round(scores.std(ddof=0), 2)) if len(scores) > 0 else 0.0

        overall_accuracy = float(np.round((df["correct_answers"].sum() / total_questions_answered) * 100, 2)) if total_questions_answered > 0 else 0.0

        best_score = float(np.round(scores.max(), 2)) if not scores.empty else 0.0
        worst_score = float(np.round(scores.min(), 2)) if not scores.empty else 0.0
